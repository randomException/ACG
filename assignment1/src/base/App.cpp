	...
	m_toneMap		(false)
{
    ...
    //Toggles to change split mode
    m_commonCtrl.addToggle((S32*)&m_settings.splitMode, FW::SplitMode_SpatialMedian, FW_KEY_NONE, "Split Mode: Spatial Median");
    m_commonCtrl.addToggle((S32*)&m_settings.splitMode, FW::SplitMode_ObjectMedian, FW_KEY_NONE, "Split Mode: Object Median");
    m_commonCtrl.addToggle((S32*)&m_settings.splitMode, FW::SplitMode_Sah, FW_KEY_NONE, "Split Mode: SAH");
    m_commonCtrl.addToggle((S32*)&m_settings.splitMode, FW::SplitMode_None, FW_KEY_NONE, "Split Mode: None");
    m_commonCtrl.addToggle((S32*)&m_settings.splitMode, FW::SplitMode_Linear, FW_KEY_NONE, "Split Mode: Linear (not implemented)");
    m_commonCtrl.addSeparator();
...
void App::constructTracer()
{
	...
	if (tryLoadHierarchy)
	{
        ...
        String(splitMode);
        switch (m_settings.splitMode)
        {
        case FW::SplitMode_SpatialMedian:
            splitMode = "SplitMode_SpatialMedian";
            break;
        case FW::SplitMode_ObjectMedian:
            splitMode = "SplitMode_ObjectMedian";
            break;
        case FW::SplitMode_Sah:
            splitMode = "SplitMode_Sah";
            break;
        case FW::SplitMode_None:
            splitMode = "SplitMode_None";
            break;
        case FW::SplitMode_Linear:
            splitMode = "SplitMode_Linear";
            //break;
        default:
            splitMode = "SplitMode_ObjectMedian";
            break;
...
#ifdef _WIN64
		String hierarchyCacheFile = String("Hierarchy-") + md5 + String("-") + splitMode + String("-x64.bin");
#else
        String hierarchyCacheFile = String("Hierarchy-") + md5 + String("-") + splitMode + String("-x86.bin");
#endif
